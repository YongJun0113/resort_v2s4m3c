...
1) 글 등록(이미지 관련 컬러 추가)
-- memberno:1 , cateno:1 이 사전에 등록되어 있어야 INSERT 가능
...
INSERT INTO contents(contentsno, memberno, cateno, title, content, web, ip, passwd, word, rdate,
                              file1, thumb1, size1)
VALUES(contents_seq.nextval, 1, 6, 'RTX3080', '사고 싶은 것', 'http://www.daum.net',
            '127.0.0.1', '123', 'gpu', sysdate, 'maple.jpg', 'maple_t.jpg', 23657);                              
...
SELECT contentsno, memberno, cateno, title, content, recom, cnt, replycnt, rdate, word, ip,
          file1, thumb1, size1
FROM contents
WHERE cateno=1 AND word LIKE '%단풍%'
ORDER BY contentsno DESC;
...
FROM (
           ...
           FROM (
                     ...
                     WHERE cateno=29 AND word LIKE '%스위스%' OR content LIKE
                     ORDER BY grpno DESC, ansnum ASC
           )          
)
WHERE r >= 1 AND r <= 10;        
