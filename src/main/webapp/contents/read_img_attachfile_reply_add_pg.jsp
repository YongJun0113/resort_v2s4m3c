...
<script type="text/javascript">
  ...
  // --------------------- 댓글 출력 관련 시작 --------------------- 
  var global_rdata = new Array(); 
  var global_rdata_cnt = 0;
    ...
    $.ajax({
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        for (i=0; i < rdata.list.length; i++) {
          ...
          // alert(row.replyno);
          var reply = new Reply(row.memberno, row.rdate, row.replyno, row.id, row.content, row.contentsno); // 객체 저장
          global_rdata.push(reply);// 배열에 저장
        }

        // alert('페이징된 댓글 목록 읽기 시작');
        list_by_contentsno_join_add_pg();
      },
      // Ajax 통신 에러, 응답 코드가 200이 아닌경우, dataType이 다른경우 
      error: function(request, status, error) { // callback 함수
        var msg = 'ERROR request.status: '+request.status + '/ ' + error;
  ...
  function list_by_contentsno_join_add_pg() {
    ...
    var start = end - 2;             // 0, 2, 4...
    var msg="";

    for (var i=start; i < end; i++) {
      global_rdata_cnt = global_rdata_cnt + 1; // DIV 태크에 글 출력 카운트
      if (global_rdata.length < global_rdata_cnt ) { // 1 page = 10개 레코드, 11개인 경우 중간 종료
        break;
      }

      var row = global_rdata[i];
