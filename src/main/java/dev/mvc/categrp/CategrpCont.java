...
public class CategrpCont {
  ...
  public ModelAndView create(CategrpVO categrpVO) {
    ...
    // mav.setViewName("/categrp/create_msg"); // /webapp/categrp/create.jsp
    
    if(cnt == 0) {
      mav.addObject("cnt", cnt); // request.setAttribute("cnt", cnt)
      mav.addObject("url", "create_msg"); // create_msg.jsp, redirect parameter 적
  ...
  /**
   * Ajax 기반 등록 처리
   * http://localhost:9090/resort/categrp/create_ajax.do
   * @return
   */
  @ResponseBody
...
//    
//    mav.setViewName("/categrp/list_ajax"); // /webapp/categrp/create.jsp
//    
//    return mav;  // forward
//  }
  ...
  public ModelAndView list(HttpSession session) {
    ...
    if (adminProc.isAdmin(session)) { // 관리자 로그인인 경우
      ...
      mav.setViewName("/categrp/list_ajax");
    } else {
 ...
 /**
  * Ajax 기반 수정 처리
  * http://localhost:9090/resort/categrp/update_ajax.do
  * @return
  */
 @ResponseBody
 ...
 public ModelAndView delete(int categrpno) {
   ...
   if(cnt == 0) {
     mav.addObject("cnt", cnt); // request.setAttribute("cnt", cnt)
     mav.addObject("url", "delete_msg"); // create_msg.jsp, redirect parameter 적
     mav.setViewName("redirect:/categrp/msg.do");
   }else {
     mav.setViewName("redirect:/categrp/list.do");
   }
